$(function () {
  ...
  let contentArray = [
    ...
    ['goods003.png', '헤리터 칼 3종 세트', '', 278000, 228000, 2054],
    ...
    ['goods009.png', 'STANDARD 180(7")', '주방용 식도', 79000, 79000, 1731],

    ['goods010.png', 'STANDARD 106(4")', '주방용 과도', 39000, 39000, 160],

    ['goods011.png', '우드그레인 키친트레이', '', 39000, 35000, 2],

    ['goods012.png', '차콜스톤 워싱바', '', 29700, 26800, 8],
  ];
    ...
    for (let j = 0; j < contentArray[i].length; j++) {
      ...
      if (j == 0) {
        galleryListArea +=
          "<td><img src='../images/" +
          contentArray[i][j] +
          "' alt='이미지' width='320'></td>";
      } else if (j == 1) {
        galleryListArea +=
          "<td class='goodsName'>" + contentArray[i][j] + '</td>';
      } else if (j == 2) {
        galleryListArea +=
          "<td class='goodsDesc'>" + contentArray[i][j] + '</td>';
      } else if (j == 3) {
        galleryListArea += "<td class='goodsPrice'>";
        galleryListArea +=
          "<span class='basicPrice numComma'>" + contentArray[i][j] + '</span>';
      } else if (j == 4) {
        galleryListArea +=
          "<span class='salePrice numComma'>" +
          contentArray[i][j] +
          '</span></td>';
      } else if (j == 5) {
        galleryListArea +=
          "<td><span class='replyCnt numComma'>" +
          contentArray[i][j] +
          '</span></td>';
      }
